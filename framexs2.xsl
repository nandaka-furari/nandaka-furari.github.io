...
<xsl:stylesheet
	...
	<!-- command -->
	<xsl:template match="*[@framexs:print='no']">
		<!--何も出力しない--></xsl:template>
	...
	<xsl:function name="framexs:getbody" as="item()*">
		...
		<xsl:choose>
			<xsl:when test="string(namespace-uri($node)) = $xhns">
				<xsl:sequence select="$node/xh:body/xh:*"></xsl:sequence>
			</xsl:when>
			<xsl:when test="string(namespace-uri($node)) = $xh2ns">
				<xsl:sequence select="$node/xh2:body/xh2:*"></xsl:sequence>
			</xsl:when>
		</xsl:choose>
	</xsl:function>
