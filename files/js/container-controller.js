...
class ContainerController {
	 ...
	 *            openChildNodes
	 */
	openComment(comment, openChildNodes) {
	 ...
	 *            closeChildNodes
	 */
	closeChildren(target, closeChildNodes) {
		target.filter();
	 ...
	 */
	uncompressComment(comment, uncompressChildNodes) {
	 ...
	 *            compressChildNodes
	 */
	compressChildComments(childComments) {
	 ...
	 *            compressChildNodes
	 */
	uncompressChildComments(childComments) {
		var childCommentList = childComments.childNodes;
		for (var int = 0; int < childCommentList.length; int++) {
			if (childCommentList.item(int).nodeType == Node.ELEMENT_NODE) {
 ...
 *            openChildNodes
 */
openChildComments(childComments) {
	var childCommentList = childComments.childNodes;
	for ( var int = 0; int < childCommentList.length; int++) {
		if (childCommentList.item(int).nodeType == Node.ELEMENT_NODE) {
 ...
 *            closeChildNodes
 */
closeChildComments(childComments) {
	var childCommentList = childComments.childNodes;
	for ( var int = 0; int < childCommentList.length; int++) {
		if (childCommentList.item(int).nodeType == Node.ELEMENT_NODE) {
 ...
 */
compressText(targetNode, length) {
 ...
 *            compressChildNodes
 */
compressComment(comment, compressChildNodes) {
	/*
	 * 要素の追加や削除はfor文の中では行わないようにしなければならない
	 */
	var number = getNumber(comment);
